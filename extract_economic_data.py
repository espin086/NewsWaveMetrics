...
def get_frequency(economic_metrics_id):
    if (
        economic_metrics_id == "DTWEXBGS"
        or economic_metrics_id == "T10Y2Y"
        or economic_metrics_id == "DFF"
    ):
        return "d"
...
def extract_all_economic_data():
    ...
    df_all = extract_economic_data(
        observation_start,
        economic_metrics_ids[0],
        get_frequency(economic_metrics_ids[0]),
    )
    ...
    for economic_metrics_id in economic_metrics_ids[1:]:
        ...
        df = extract_economic_data(
            observation_start, economic_metrics_id, get_frequency(economic_metrics_id)
        )
...
if __name__ == "__main__":
    df_all = extract_all_economic_data()
    print(df_all)
